...
export const PeriodSuffix: React.FC<{ dataPoints: number; maxPeriod?: number }> = ({ dataPoints, maxPeriod = 30 }) => {
  ...
  const handleToggle = (e: React.PointerEvent) => {
    if (e.pointerType === 'touch') { e.preventDefault(); setOpen(o => !o); }
  };
  return (
    <span className="ml-1">
      <TooltipPrimitive.Provider delayDuration={300}>
        <TooltipPrimitive.Root open={open} onOpenChange={setOpen}>
          <TooltipPrimitive.Trigger asChild>
            <span className="text-[11px] font-medium text-[#a0a0b0] cursor-help" tabIndex={0} onPointerDown={handleToggle}>
              ({dataPoints}d)
            </span>
          </TooltipPrimitive.Trigger>
          <TooltipPrimitive.Portal>
