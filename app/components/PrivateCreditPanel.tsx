...
/**
 * PrivateCreditPanel — Control Room tool for tagging products as Private Credit.
 * Tagged products will receive special rules on the single product page.
 * Pattern follows HiddenProductsPanel: grouped by ticker, click-to-toggle, batch save.
 */
import React, { useState, useEffect, useCallback, useMemo } from 'react';
import {
  Landmark, Search, X, RefreshCw, AlertCircle, ChevronDown, ChevronUp,
  Check,
} from 'lucide-react';
...
export const PrivateCreditPanel: React.FC = () => {
  const [products, setProducts] = useState<Product[]>([]);
  const [taggedIds, setTaggedIds] = useState<Set<number | string>>(new Set());
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const [allProducts, pcData] = await Promise.all([
        api.fetchPoolsIncludingHidden(),
        api.fetchPrivateCreditProducts(),
      ]);
      setProducts(allProducts);
      setTaggedIds(new Set(pcData.ids || []));
      setPendingChanges(false);
    } catch (err: any) {
      console.error('[PrivateCredit] Fetch error:', err);
  ...
  const toggleTagged = (id: number | string) => {
    setTaggedIds(prev => {
  ...
  const saveChanges = async () => {
    try {
      setSaving(true);
      const result = await api.updatePrivateCreditProducts(Array.from(taggedIds));
      // Verify server actually stored all IDs — warn if mismatch
      if (result.count !== taggedIds.size) {
        console.warn(`[PrivateCredit] ID mismatch: sent ${taggedIds.size}, server stored ${result.count}. Server IDs:`, result.ids);
        toast.warning(`Saved ${result.count} of ${taggedIds.size} tags. Some IDs may not have been stored correctly.`);
      } else {
        toast.success(`${taggedIds.size} product${taggedIds.size !== 1 ? 's' : ''} tagged as Private Credit`);
      }
      setPendingChanges(false);
    } catch (err: any) {
      console.error('[PrivateCredit] Save error:', err);
      toast.error('Failed to save Private Credit tags');
    } finally {
      setSaving(false);
    }
  };
  ...
  const filteredGrouped = useMemo(() => {
    ...
    for (const [ticker, items] of Object.entries(grouped)) {
      const filtered = items.filter(p => {
        if (filterMode === 'tagged' && !taggedIds.has(p.id)) return false;
        if (filterMode === 'untagged' && taggedIds.has(p.id)) return false;
        if (!q) return true;
        return (
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold text-foreground">Private Credit</h2>
            <p className="text-muted-foreground text-xs font-medium">
              {taggedCount} tagged out of {products.length} total products
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          {pendingChanges && (
            <button
              onClick={saveChanges}
              disabled={saving}
              className="flex items-center gap-1.5 px-5 py-2 bg-indigo-500 text-white rounded-xl text-xs font-semibold hover:bg-indigo-500/90 transition-all disabled:opacity-50 shadow-sm"
            >
              {saving ? (
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-9 pr-8 py-2 bg-muted/50 border border-border rounded-xl text-xs focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all"
          />
          {searchQuery && (
            <button
        ...
        <div className="flex items-center gap-1 p-1 bg-muted/50 rounded-xl border border-border">
          {(['all', 'tagged', 'untagged'] as const).map(mode => (
            <button
              key={mode}
            ...
            >
              {mode === 'all' ? `All (${products.length})`
                : mode === 'tagged' ? `Tagged (${taggedCount})`
                : `Untagged (${products.length - taggedCount})`}
            </button>
          ))}
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          ...
          {tickers.map(ticker => {
            ...
            const tickerTaggedCount = items.filter(p => taggedIds.has(p.id)).length;
              ...
              <div key={ticker} className="bg-card rounded-2xl border border-border overflow-hidden">
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    {tickerTaggedCount > 0 && (
                      <span className="px-2 py-0.5 bg-indigo-500/10 rounded-lg text-[10px] font-semibold text-indigo-500">
                        {tickerTaggedCount} tagged
                      </span>
                    )}
                  </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2 min-w-0">
                            <span className={`text-xs font-medium truncate ${isTagged ? 'text-indigo-600 dark:text-indigo-400' : 'text-foreground'}`}>
                              {p.product_name}
                            </span>
                            {p.curator && p.curator !== '-' && (
                          ...
                          <div className="md:hidden flex items-center gap-2 text-[10px] text-muted-foreground font-medium">
                            ...
                            <span className="ml-auto">
                              {isTagged
                                ...
                                : <span className="w-3.5 h-3.5 rounded border border-border/60 block" />}
                            </span>
                          </div>
                          {/* Network */}
                          ...
                          <span className="hidden md:flex items-center justify-center">
                            {isTagged ? (
                              <div className="w-5 h-5 rounded-md bg-indigo-500 flex items-center justify-center group-hover:bg-indigo-600 transition-colors">
                                <Check className="w-3 h-3 text-white" />
                              </div>
                            ) : (
                              <div className="w-5 h-5 rounded-md border-2 border-border/60 group-hover:border-indigo-400 transition-colors" />
                            )}
                          </span>
                        </div>
                      );
