/**
 * EarningsCalculator — simple "what could I earn" widget.
 * Pre-defined deposit presets + custom input.
 * Uses current APY for projections. Educational only.
 */
import React, { useState, useMemo } from 'react';
import { Calculator } from 'lucide-react';
import { TooltipLabel } from '../ui/TooltipLabel';
import { EARNINGS } from '@/app/utils/tooltipDefs';
  ...
  return (
    <div>
          ...
          <button
            ...
            className={`py-1.5 text-[12px] font-medium rounded-[8px] border transition-all ${
              selectedPreset === amount
                ...
                : 'bg-[#f8f8fa] dark:bg-white/[0.04] border-[#eff0f4] dark:border-white/[0.08] text-[#6b6b7b] dark:text-foreground/50 hover:border-[#d0d0d8] dark:hover:border-white/15'
            }`}
          >
      ...
      <p className="mt-3 text-[10px] leading-relaxed text-[#52525b] dark:text-foreground/40 font-normal">
        {isPrivateCredit
          ? `For illustrative purposes only. Based on the current ${apy.toFixed(2)}% on-chain APY for ${ticker.toUpperCase()}. This is a fixed-rate product. Projected earnings assume the contractual rate of ${apy.toFixed(2)}% is maintained. Actual returns depend on the borrower honoring the agreed rate.`
