/**
 * HomepageContent — SEO-rich content sections below the asset table.
 *
 * Sections:
 *   1. "Top DeFi Yields Right Now" — answer capsule with top strategy per asset
 *   2. "Everything You Need to Compare Yields" — Figma-inspired green showcase cards
...
/** Top 8 projects by hardcoded display order, with dynamic data */
const TOP_PROJECTS = [
...
/** Top strategy per asset — highest spotAPY with TVL > $10k */
  ...
  return order
    ...
    .concat(
      [...map.entries()]
        ...
        .sort((a, b) => b[1].spotAPY - a[1].spotAPY)
        .map(([, p]) => p),
    );
};

/** Strip parenthetical annotations like "(Private Credit)" from product names */
const cleanName = (name: string) => name.replace(/\s*\(.*?\)\s*/g, '').trim();

// ── Top DeFi Yields Section ──────────────────────────────────
...
export const TopDeFiYields: React.FC<Props> = ({ products }) => {
  ...
  return (
    <section id="top-defi-yields">
      ...
      {/* Compact yield rows — clean and data-focused */}
      <div className="rounded-2xl border border-[#e6e8ea] dark:border-border overflow-hidden bg-white dark:bg-card divide-y divide-[#f0f1f3] dark:divide-border/50">
        {tops.slice(0, 6).map(p => {
          ...
          const networkIcon = resolveNetworkIcon(p.network);
...
// ── Mini UI Previews for showcase cards ──────────────────────

/** Realistic mini chart for the "Performance" card */
const MiniChartPreview = () => (
  <div className="bg-white rounded-xl p-5 shadow-[0_1px_3px_rgba(0,0,0,0.06)]" style={{ fontFamily: FONT }}>
    ...
    <svg viewBox="0 0 280 80" className="w-full h-[80px]" preserveAspectRatio="none">
      ...
      <path
        d="M0,60 C20,55 40,50 60,42 C80,34 100,38 120,30 C140,22 160,28 180,20 C200,12 220,18 240,10 C260,6 270,8 280,5"
        fill="none" stroke="#08a671" strokeWidth="2"
      />
      <path
        d="M0,60 C20,55 40,50 60,42 C80,34 100,38 120,30 C140,22 160,28 180,20 C200,12 220,18 240,10 C260,6 270,8 280,5 L280,80 L0,80 Z"
        fill="url(#chartFill)"
      />
    </svg>
...
/** Realistic mini table for the "Compare" card */
const MiniTablePreview = () => (
  <div className="bg-white rounded-xl shadow-[0_1px_3px_rgba(0,0,0,0.06)] overflow-hidden" style={{ fontFamily: FONT }}>
    {/* Header row */}
    <div className="grid grid-cols-[1fr_70px_70px] px-4 py-2.5 bg-[#fafafa] border-b border-[#f0f1f3]">
      <span className="text-[10px] font-semibold text-[#99a0aa] uppercase tracking-wider">Strategy</span>
      <span className="text-[10px] font-semibold text-[#99a0aa] uppercase tracking-wider text-right">APY</span>
      <span className="text-[10px] font-semibold text-[#99a0aa] uppercase tracking-wider text-right">TVL</span>
    </div>
    {[
      { name: 'Flagship USDC', platform: 'Morpho', apy: '8.42%', tvl: '$142M', top: true },
      { name: 'Gauntlet USDC Core', platform: 'Morpho', apy: '6.18%', tvl: '$89M', top: false },
      { name: 'USDC Lending', platform: 'Aave V3', apy: '5.31%', tvl: '$1.2B', top: false },
      { name: 'Steakhouse USDC', platform: 'Morpho', apy: '4.87%', tvl: '$67M', top: false },
    ].map((row) => (
      <div
        key={row.name}
        className={`grid grid-cols-[1fr_70px_70px] px-4 py-2.5 border-b border-[#f7f8f9] last:border-b-0 ${row.top ? 'bg-[#f0faf6]/50' : ''}`}
      >
        <div className="min-w-0">
          <div className={`text-[11px] font-medium truncate ${row.top ? 'text-[#08a671]' : 'text-[#30313e]'}`}>{row.name}</div>
          <div className="text-[9px] text-[#b0b5bd]">{row.platform}</div>
        </div>
        <span className={`text-[11px] font-semibold tabular-nums text-right self-center ${row.top ? 'text-[#08a671]' : 'text-[#0e0f11]'}`}>{row.apy}</span>
        <span className="text-[11px] text-[#77808d] tabular-nums text-right self-center">{row.tvl}</span>
      </div>
    ))}
  </div>
);

/** Realistic mini vault page for the "Direct deposit" card */
const MiniVaultPreview = () => (
  <div className="bg-white rounded-xl p-5 shadow-[0_1px_3px_rgba(0,0,0,0.06)]" style={{ fontFamily: FONT }}>
    ...
    <button
      ...
      style={{
        background: '#08a671',
        boxShadow: 'inset 0px -3px 0px 0px rgba(0,0,0,0.18)',
      }}
    >
      Open Product
    </button>
...
/** Mini multi-chain preview for the "Coverage" card */
const MiniMultiChainPreview = () => {
  const { resolveNetworkIcon } = useRegistry();
  const chains = [
    ...
    <div className="grid grid-cols-3 gap-5">
      {chains.map(c => (
        <div key={c.id} className="flex flex-col items-center gap-2">
          <img src={resolveNetworkIcon(c.id)} alt={c.name} className="w-10 h-10 object-contain" />
          <span className="text-[11px] font-medium text-white/80">{c.name}</span>
        </div>
      ))}
    </div>
  );
...
export const HomepageContent: React.FC<Props> = ({ products }) => {
  ...
  /** Top 5 assets ranked by number of strategies (TVL > $10k filter) */
  ...
  /** Top 8 projects with dynamic strategy count & best APY */
  const projectCards = useMemo(() => {
    return TOP_PROJECTS.map(proj => {
      const prods = products.filter(p => projectToSlug(p.platform_name) === proj.slug);
      const topAPY = prods.length > 0 ? Math.max(...prods.map(p => p.spotAPY || 0)) : 0;
      return {
        ...proj,
        strategyCount: prods.length,
        topAPY,
      };
  ...
  return (
    <div className="space-y-14">
      ...
      {/* ═══ Feature Showcase — Figma-inspired green cards ═══ */}
      <section>
        <h2
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div
            ...
            style={{ background: 'rgba(8,166,113,0.88)' }}
          >
            ...
            <div className="px-7 sm:px-8 pb-7 sm:pb-8 pt-2">
              <h3
                className="text-[17px] font-semibold text-white leading-snug tracking-[-0.02em]"
                style={{ fontFamily: FONT }}
              >
                30-day yield history for every strategy
              </h3>
              <p
                className="text-[14px] text-white/65 leading-[1.6] mt-2"
                style={{ fontFamily: FONT }}
              >
          ...
          {/* ── Card 2: Compare strategies ── */}
          <div
            className="rounded-2xl overflow-hidden flex flex-col"
            style={{ background: 'rgba(8,166,113,0.82)' }}
          >
            <div className="p-5 sm:p-7 flex-1 flex items-center">
              <div className="w-full">
                {/* Mini asset ranking table — mirrors actual AssetGrid */}
                <div className="bg-white rounded-xl shadow-[0_1px_3px_rgba(0,0,0,0.06)] overflow-hidden" style={{ fontFamily: FONT }}>
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="border-b border-[#eef0f2]">
                        ...
                        <th className="px-2 py-2.5 text-[9px] font-semibold text-[#99a0aa] uppercase tracking-[0.12em] text-right">Highest APY</th>
                        <th className="w-[24px] pr-3 py-2.5" />
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-[#f5f6f7]">
                      {assetRanking.map(row => {
                        const rowIcon = resolveAssetIcon(row.ticker);
                        return (
                        <tr key={row.ticker} className="group/row">
                          <td className="pl-4 pr-1 py-2.5 text-[10px] text-[#b0b5bd] font-normal">{row.rank}</td>
                          <td className="pl-2 pr-1 py-2.5">
                            <div className="flex items-center gap-2">
                              <div className="w-5 h-5 flex items-center justify-center shrink-0">
                                {rowIcon ? (
                                  <img src={rowIcon} alt={row.ticker} className="w-5 h-5 object-contain" />
                                ) : (
                          ...
                          <td className="pr-3 py-2.5 text-right">
                            <svg width="10" height="10" viewBox="0 0 24 24" fill="none" className="inline-block text-[#d0d5dd]">
          ...
          {/* ── Card 3: One click to deposit ── */}
          <div
            className="rounded-2xl overflow-hidden flex flex-col"
            style={{ background: 'rgba(8,166,113,0.75)' }}
          >
          ...
          {/* ── Card 4: Multi-chain coverage ── */}
          <div
            className="rounded-2xl overflow-hidden flex flex-col"
            style={{ background: 'rgba(8,166,113,0.70)' }}
          >
            <div className="p-5 sm:p-7 flex-1 flex items-center">
              <div className="w-full">
                <MiniMultiChainPreview />
              </div>
            </div>
            <div className="px-7 sm:px-8 pb-7 sm:pb-8 pt-2">
              <h3
              ...
              <p
                ...
                style={{ fontFamily: FONT }}
              >
                Ethereum, Base, Arbitrum, Optimism, Polygon, and Sonic — all indexed and normalized in one place.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-10 sm:gap-16 lg:gap-24">
          {([
            ...
            <div key={s.label} className="flex items-center gap-10 sm:gap-16 lg:gap-24">
              {i > 0 && (
                <div className="hidden sm:block w-px h-12 bg-[#e0e3e7] dark:bg-border" />
              )}
